...
object BrightnessUtils {
     ...
     */
    fun convertGammaToLinear(`val`: Float): Float {
        val ret: Float = if (`val` <= R) {
            MathUtils.sq(`val` / R)
        } else {
            MathUtils.exp((`val` - C) / A) + B
        }

        // HLG is normalized to the range [0, 12], ensure that value is within that range,
        // it shouldn't be out of bounds.
        val normalizedRet: Float = MathUtils.constrain(ret, 0f, 12f)
     ...
     */
    fun convertLinearToGamma(`val`: Float): Float {
        // For some reason, HLG normalizes to the range [0, 12] rather than [0, 1]
        val normalizedVal = `val` * 12
